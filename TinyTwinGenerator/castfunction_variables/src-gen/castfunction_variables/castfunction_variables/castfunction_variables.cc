 ...
 *
 * Source: file:/home/ali123ata/Desktop/3/new/Tinification/TinyTwinGenerator/castfunction_variables/castfunction_variables.lf
 */

#include "castfunction_variables/castfunction_variables.hh"
...
// outer constructor
castfunction_variables::castfunction_variables(const std::string& name, reactor::Environment* __lf_environment, Parameters&& __lf_parameters)
...
castfunction_variables::castfunction_variables(const std::string& name, reactor::Reactor* __lf_container, Parameters&& __lf_parameters)
...
// inner constructor
castfunction_variables::Inner::Inner(::reactor::Reactor* reactor, Parameters&& parameters)
  : LFScope(reactor)
  ...
  , advancesTime(0)
  , cntShift(0)
  , stateId_varValue(10000000)
  , stateId_varName(10000000)
  , index(-1)
{}

void castfunction_variables::assemble() {
  // reaction_1
...
void castfunction_variables::Inner::reaction_1([[maybe_unused]] const reactor::StartupTrigger& startup) {
    ...
    std::string filename;
    	...
    	*/
    	...
    	// --- Predefined settings for TinyTwin extraction ---
  	...
  	// std::cout << i << '\n';
    		...
    		for(long i = 0; i <= index; i++){
                   if(stateId_varValue[i][0] == srcNumber){
                      //replace title of transition with msg from Q
                       for(long msg = 1; msg < stateId_msg[i].size(); msg++){
  	...
  	if (annotateVar == "Y") {
  		// Try to swap the XML title with the actual queued message name (case-insensitive)
  		if (ignoreListTrans.find(title) == std::string::npos) {
  			for (long i = 0; i <= index; ++i) {
  				if (stateId_varValue[i][0] == srcNumber) {
  					for (long msg = 1; msg < (long)stateId_msg[i].size(); ++msg) {
  						std::string cand = stateId_msg[i][msg];
  						for (char &ch : cand) ch = std::tolower(static_cast<unsigned char>(ch));
  						if (cand.find(title) != std::string::npos) {
  							title = stateId_msg[i][msg];   // keep original form like m2()
  							break;
  						}
  					}
  					break;
  		...
  		// Build "owner.msg", normalize case, convert () -> [] on the message, then append ".[]"
  		title = owner + "." + title;
  
  		for (char &ch : title) ch = std::tolower(static_cast<unsigned char>(ch));
  		for (std::size_t k = 0; k < title.size(); ++k) {
  			if (title[k] == '(')      title[k] = '[';
  			else if (title[k] == ')') title[k] = ']';
  		}
  
  		// Always empty variable-annotation
  		title += ".[]";
  	}
                   ...
                   if(stateId_varValue[i][0] == srcNumber){
                       for(long m = 0; m <= index; m++){
                           if(stateId_varValue[m][0] == destNumber){
                               for(long v = 1; v < stateId_varValue[i].size(); v++){
    		...
    		*/
    		...
    		/*
    		for(long j = 0; j < state_Id.size(); j++){
  ...
  // std::cout<<"after big for" << '\n';
    	...
    	*/
    	...
    	for(long i = 0; i < state_Id.size(); i++){
