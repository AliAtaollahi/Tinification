...
def abstractDotByLine(dot: String): String = {
  ...
  // drop all edges whose destination is METHOD/METHOD_RETURN
  val methodLikeRepIds = repByLine.values.filter(n => n.kind.startsWith("METHOD")).map(_.id).toSet
  ...
  val edgeLines =
    edgeRx.findAllMatchIn(dot).flatMap { m =>
      ...
      if (src != dst && repIds(src) && repIds(dst) && !methodLikeRepIds(dst)) Some(s""""$src" -> "$dst"""") else None
    }.toSet.toList
