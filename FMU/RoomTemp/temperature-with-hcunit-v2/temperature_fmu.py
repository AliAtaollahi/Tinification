...
class Temperature(Fmi2Slave):
    ...
    Room temperature FMU with:
      ...
      - modular heater: enable/disable with parameter `heater_enabled`
    ...
    Actual room temp T follows:
      ...
      heater    = 1 if T < 20, 0 if T > 21, else keep previous state
                  (only if heater_enabled = 1)
    """

    author = "Ali"
    description = "Smooth room temp with stochastic environment and modular heater"
    ...
    def __init__(self, **kwargs):
        ...
        self.noise = 1.0                 # noise state (roughly order 1)
        ...
        # heater state (output) and modular on/off flag (parameter)
        self.heater = 0                  # 0 = off, 1 = on
        self.heater_enabled = 0          # 1 = heater active, 0 = heater always off
        ...
        # modular heater enable flag (parameter)
        self.register_variable(
            Integer(
                "heater_enabled",
                ...
                description="Enable heater logic (0 = disabled, 1 = enabled)",
            )
        )
    ...
    def do_step(self, current_time, step_size):
        ...
        for _ in range(n_sub):
            ...
            if self.heater_enabled:  # heater logic active
